/*
 * qTip2 - Pretty powerful tooltips - v2.0.1-65-
 * http://qtip2.com
 *
 ...
 *
 * Date: Sat Apr 6 2013 07:50 UTC+0000
...
(function( factory ) {
	"use strict";
	// TODO: Uncomment lines below when lobby fully supports AMD
...
if(!$.curCSS) {
	...
	// This is needed to make qTip2 work with IE8.
	// Identical to qtip/vendorCss except for _.each
	vendorCss = function(elem, prop) {
		var ucProp = prop.charAt(0).toUpperCase() + prop.slice(1),
			props = (prop + ' ' + cssPrefixes.join(ucProp + ' ') + ucProp).split(' '),
			cur, val, i;
		...
		var result;
		_.each(props, function (cur) {
			if((val = elem.css(cur)) !== undefined) {
				result = cssProps[prop] = cur, val;
			}
		});
		return result;
	};
